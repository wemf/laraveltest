...
<div class="x_panel">
  ...
  <div class="x_content">
    @include('Trasversal.Boton.botonCrud', ['href' => "/clientes/caja/create"])
...
@section('javascript')   
  @parent
   column=[           
  	...
  	dataTableActionFilter("{{url('/clientes/caja/get')}}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column)

    $("#updateAction1").click(function() {
      var url2="{{ url('/clientes/caja/update') }}";
      updateRowDatatableAction(url2)
    });
    

    $("#deletedAction1").click(function() { 
      var url2="{{ url('/clientes/caja/delete') }}";
      deleteRowDatatableAction(url2);
    });

    $("#activatedAction1").click(function() { 
      var url2="{{ url('/clientes/caja/active') }}";
