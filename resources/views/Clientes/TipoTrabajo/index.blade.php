...
<div class="x_panel">
  ...
  <div class="x_content">
    @include('Trasversal.Boton.botonCrud', ['href' => "/clientes/tipo/trabajo/create"])
    ...
    <div class="contentfilter-table">
        <table cellpadding="3" class="table-filter" cellspacing="0" border="0" style="width: 100%; margin: 30px 10px;">
            <tbody>
                ...
                <tr id="filter_col4" class="no-width" data-column="1">
                    <td>
...
@section('javascript')   
  @parent
   column=[           
  	...
  	dataTableActionFilter("{{url('/clientes/tipo/trabajo/get')}}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column)

    $("#updateAction1").click(function() {
      var url2="{{ url('/clientes/tipo/trabajo/update') }}";
      updateRowDatatableAction(url2)
    });

    $("#deletedAction1").click(function() { 
      var url2="{{ url('/clientes/tipo/trabajo/delete') }}";
      deleteRowDatatableAction(url2);
    });
    $("#activatedAction1").click(function() { 
      var url2="{{ url('/clientes/tipo/trabajo/active') }}";
