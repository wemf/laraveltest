...
<div class="x_panel">
  <div class="x_title">
    <h2>Motivos de Retiro</h2>
    <div class="clearfix"></div>
  </div>
  <div class="x_content">
    @include('Trasversal.Boton.botonCrud', ['href' => "/clientes/motivoretiro/create"])
...
@section('javascript')   
  @parent
   column=[           
  	...
  	dataTableActionFilter("{{url('/clientes/motivoretiro/get')}}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column)

    $("#updateAction1").click(function() {
      var url2="{{ url('/clientes/motivoretiro/update') }}";
      updateRowDatatableAction(url2)
    });

    $("#deletedAction1").click(function() { 
      var url2="{{ url('/clientes/motivoretiro/delete') }}";
      deleteRowDatatableAction(url2);
    });

    $("#activatedAction1").click(function() { 
      var url2="{{ url('/clientes/motivoretiro/active') }}";
