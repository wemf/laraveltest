...
<div class="x_panel">
  ...
  <div class="x_content">
    @include('Trasversal.Boton.botonCrud', ['href' => "/clientes/eps/create"])
...
@section('javascript')   
  @parent
   column=[           
  	...
  	dataTableActionFilter("{{url('/clientes/eps/get')}}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column)

    $("#updateAction1").click(function() {
      var url2="{{ url('/clientes/eps/update') }}";
      updateRowDatatableAction(url2)
    });
    

    $("#deletedAction1").click(function() { 
      var url2="{{ url('/clientes/eps/delete') }}";
      deleteRowDatatableAction(url2);
    });

    $("#activatedAction1").click(function() { 
      var url2="{{ url('/clientes/eps/active') }}";
