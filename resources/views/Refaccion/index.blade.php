...
<div class="x_panel">
  <div class="x_title"><h2>Refacción</h2>
    <div class="clearfix"></div>
  </div>
  <div class="x_content">
    <div class="btn-group pull-right espacio botonera-ref" role="group" aria-label="..." >
    ...
    <table id="table_refaccion" class="display totales_resolucion" width="100%" cellspacing="0">
        <thead>
            <tr>
                <th>
...
@push('scripts')
  <script src="{{ asset('/js/OrdenResolucion/refaccion.js') }}"></script>
...
@section('javascript')
  ...
  //<script> 
    ...
    datatableRefaccion('table_refaccion', "{{ url('/contrato/refaccion/get') }}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column);

    $("#newAction").click(function() {
        var ordenes = "";
        var id_tienda = $('#col0_filter').val();
        $('#table_refaccion .check-resolucionar:checked').each(function(){
            ordenes += $(this).parent().parent().parent().attr('id') + '-';
        });

        var url2="{{ url('/contrato/refaccion/refaccionar') }}/"+id_tienda+"/"+ordenes.slice(0,-1)+"/1";
    ...
    $("#viewAction").click(function() {
        ...
        var url2="{{ url('/contrato/refaccion/refaccionar') }}/"+id_tienda+"/"+ordenes.slice(0,-1)+"/2";
        if (ordenes != null && ordenes != "") {
            window.location = url2;
        } else {
            Alerta('Error', 'Seleccione un registro.', 'error');
