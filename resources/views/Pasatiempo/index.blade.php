...
<div class="x_panel">
  ...
  <div class="x_content">
    @include('Trasversal.Boton.botonCrud', ['href' => "/pasatiempo/create"])
...
@section('javascript')   
  @parent
   column=[           
  	...
  	dataTableActionFilter("{{url('/pasatiempo/get')}}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column)

    $("#updateAction1").click(function() {
      var url2="{{ url('/pasatiempo/update') }}";
      updateRowDatatableAction(url2)
    });

    $("#deletedAction1").click(function() { 
      var url2="{{ url('/pasatiempo/delete') }}";
      deleteRowDatatableAction(url2);
    });

    $("#activatedAction1").click(function() { 
      var url2="{{ url('/pasatiempo/active') }}";
