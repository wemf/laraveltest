...
<div class="x_panel">
    ...
    <div class="x_content">
        <div class="btn-group pull-right espacio" role="group" aria-label="...">
            <a href="{{ url('/configcontrato/valorventa/create') }}" title="Agregar Nuevo Registro" type="button" class="btn btn-success"><i class="fa fa-plus  "></i> Nuevo</a>
            ...
            <table cellpadding="3" class="table-filter" cellspacing="0" border="0" style="width: 100%; margin: 30px 10px;">
                <tbody>
                    ...
                    <tr id="filter_col2" class="no-width" data-column="1">
                        <td>
                            Inactivos
                            <input type="checkbox" onchange="intercaleCheckInvert(this);" id="col1_filter" class="column_filter check-control check-pos" value="1" />
        ...
        <table id="dataTableAction" class="display" width="100%" cellspacing="0">
            <thead>
                <tr>
                    ...
                    <th>Valor por Unidad para la Venta</th>
                    <th>Activo</th>	
                </tr>
            </thead>
        </table>
...
@parent 
    column=
        [ 
            ...
            { "data": "tienda" },
            { "data": "categoria" },
            { "data": "nombre_concatenado" }, 
            { "data": "medida_peso"}, 
            { "data": "valor_venta_x_1" }, 
            { "data": "estado" },
        ]; 
dataTableActionFilter("{{url('/configcontrato/valorventa/get')}}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column);
loadSelectInput("#col0_filter", "{{url('/products/categories/getCategory')}}",2); 
$("#updateAction1").click(function() {
    var url2="{{ url('/configcontrato/valorventa/edit') }}"; 
    updateRowDatatableAction(url2) 
}); 
$("#deletedAction1").click(function(){ 
    var url2="{{ url('/configcontrato/valorventa/inactive') }}"; 
    deleteRowDatatableAction(url2); 
}); 
$("#deletedAction2").click(function(){ 
    var url2="{{ url('/configcontrato/valorventa/delete') }}"; 
    deleteRowDatatableAction(url2, "¿Eliminar el registro?"); 
}); 
$("#activatedAction1").click(function() { 
    var url2="{{ url('/configcontrato/valorventa/active') }}"; 
