...
<div class="x_panel">
  ...
  <div class="x_content">
    @include('Trasversal.Boton.botonCrud', ['href' => "/departamento/create"])
        ...
        <table cellpadding="3" class="table-filter" cellspacing="0" border="0" style="width: 100%;">
            <tbody>
                ...
                <tr id="filter_col2" data-column="1">
                    <td>Departamento<input type="text" class="column_filter form-control" id="col1_filter"></select></td>
                </tr>
...
@section('javascript')   
  @parent
   column=[           
            { "data": "pais" },
            { "data": "nombre" },
            { "data": "indicativo_departamento" },
            { "data": "codigo_dane"},
            { "data": "estado" },
        ];
  	dataTableActionFilter("{{url('/departamento/get')}}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column)

      loadSelectInput("#col0_filter","{{ url('/pais/getSelectList') }}")
      SelectValPais("#col0_filter");
   
    $("#updateAction1").click(function() {
      var url2="{{ url('/departamento/update') }}";
      updateRowDatatableAction(url2)
    });

    $("#deletedAction1").click(function() { 
      var url2="{{ url('/departamento/delete') }}";
      deleteRowDatatableAction(url2);
    });

    $("#activatedAction1").click(function() { 
      var url2="{{ url('/departamento/active') }}";
