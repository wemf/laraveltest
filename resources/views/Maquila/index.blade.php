...
<form id="frm_reporte_pdf" class="hide" method="POST" action="{{ url('contrato/maquila/generatepdf') }}">
    {{ csrf_field() }}
...
<div class="x_panel">
  <div class="x_title"><h2>Órdenes de maquila</h2>
  ...
  <div class="x_content">
    ...
    <table id="table_maquila" class="display totales_resolucion" width="100%" cellspacing="0">
        <thead>
            <tr>
                <th>
                    <input type="checkbox" onchange="intercaleCheck(this);" id="all_check" class="check-control check-pos" value="0" />
                    <label for="all_check" onclick="resetChecks();" class="lbl-check-control" style="font-size: 20px!important; font-weight: 100; margin: 0px;"></label>
                </th>      
                <th> </th>  
...
@push('scripts')
  <script src="{{ asset('/js/OrdenResolucion/maquila.js') }}"></script>
...
@section('javascript')
  ...
  //<script> 
    ...
    column=[
        ...
        datatableRefaccion('table_maquila', "{{ url('/contrato/maquila/get') }}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column)
    ...
    $("#newAction").click(function() {
        ...
        $('#table_maquila .check-resolucionar:checked').each(function(){
            ordenes += $(this).parent().parent().parent().attr('id') + '-';
        });

        var url2="{{ url('/contrato/maquila/maquilaar') }}/"+id_tienda+"/"+ordenes.slice(0,-1)+"/1";
    ...
    $("#viewAction").click(function() {
        ...
        var url2="{{ url('/contrato/maquila/maquilaar') }}/"+id_tienda+"/"+ordenes.slice(0,-1)+"/2";
        if (ordenes != null && ordenes != "") {
            window.location = url2;
        } else {
            Alerta('Error', 'Seleccione un registro.', 'error');
