...
<div class="x_panel">
  ...
  <div class="x_content">  
    ...
    @include('Trasversal.Boton.botonCrud', ['href' => "/pais/create"])
...
@section('javascript')   
  @parent
   column=[           
            { "data": "nombre" },
            { "data": "abreviatura" },
            { "data": "codigo_telefono" },
            { "data": "estado" },
        ];
  	dataTableActionFilter("{{url('/pais/get')}}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column)

    $("#updateAction1").click(function() {
      var url2="{{ url('/pais/update') }}";
      updateRowDatatableAction(url2)
    });
    

    $("#deletedAction1").click(function() { 
      var url2="{{ url('/pais/delete') }}";
      deleteRowDatatableAction(url2);
    });

    $("#activatedAction1").click(function() { 
      var url2="{{ url('/pais/active') }}";
