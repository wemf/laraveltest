...
<!-- END MODAL REPORTES -->
<div class="x_panel">
  <div class="x_title"><h2>Pre-refacción</h2>
    <div class="clearfix"></div>
  </div>
  <div class="x_content">
    <div class="btn-group pull-right espacio" role="group" aria-label="..." >
      <a title="Procesar" id="newAction" class="btn btn-success btn-procesar"><i class="fa fa-paper-plane  "></i> Procesar</a>
      <a title="Ver" id="viewAction" class="btn btn-primary btn-ver-index"><i class="fa fa-eye"></i> Ver</a>
      <button type="button" id="btn-reporte" class="btn btn-warning"><i class="fa fa-file-pdf-o"></i> Generar Reportes</button>
        ...
        <table cellpadding="3" class="table-filter" cellspacing="0" border="0" style="width: 100%;">
            <tbody>
                <tr id="filter_col1" data-column="0">
                    <td>Joyería
                      <select  class="column_filter form-control slc_tienda" name="tienda" id="col0_filter" disabled data-load="@if($tienda != null){{ $tienda->id }}@endif"></select>
                    </td>
                </tr>
                ...
                <tr id="filter_col3" data-column="2">
                    <td>Estado
                        <select  class="column_filter form-control filter_estado" name="estado" id="col2_filter">
    ...
    <div class="row">
        <div class="panel panel-default">
            <div class="panel-body">
                ...
                Cantidad Items: <label id="lbl_total_cantidad_items">0</label>
            </div>
        </div>
    </div>

      <table id="table_prerefaccion" class="display totales_resolucion" width="100%" cellspacing="0" > 
         <thead>
            <tr>
                <th>
                ...
                <th>Valor Total</th>
                <th>Peso total</th>
                <th>Peso estim. total</th>
                <th>Peso taller</th>
                <th class="hide">Cantidad Items</th>
                <th>Estado</th>
...
@section('javascript')   
  ...
  //<script>    
    ...
    column=[  
        ...
        datatableRefaccion('table_prerefaccion', "{{ url('/contrato/prerefaccion/get') }}","{{url('/plugins/datatable/DataTables-1.10.13/json/spanish.json')}}",column)
    ...
    $("#newAction").click(function() {
      ...
      $('#table_prerefaccion .check-resolucionar:checked').each(function(){
        ordenes += $(this).parent().parent().parent().attr('id') + '-';
      });
      
      var url2="{{ url('/contrato/prerefaccion/prerefaccionar') }}/"+id_tienda+"/"+ordenes.slice(0,-1)+"/1";
    ...
    $("#viewAction").click(function() {
        ...
        var url2="{{ url('/contrato/prerefaccion/prerefaccionar') }}/"+id_tienda+"/"+ordenes.slice(0,-1)+"/2";
        if (ordenes != null && ordenes != "") {
            window.location = url2;
        } else {
            Alerta('Error', 'Seleccione un registro.', 'error');
