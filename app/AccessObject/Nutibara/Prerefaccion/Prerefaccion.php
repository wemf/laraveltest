...
namespace App\AccessObject\Nutibara\Prerefaccion;

use App\Models\Nutibara\OrdenResolucion\OrdenResolucion AS ModelPrerefaccion;
...
class Prerefaccion 
{
	public static function get($start, $end, $colum, $order, $search)
	{
		...
		return DB::table('tbl_orden')->leftJoin('tbl_orden_hoja_trabajo_cabecera', function($join){
									...
									->where('tbl_orden.proceso',env('PROCESO_PREREFACCION'))
	...
	public static function countPrerefaccion( $search){
	...
	public static function getPrerefaccionById($id){
		return ModelPrerefaccion::where('id',$id)->first();
	}
	...
	{	
		...
		for($i=0; $i < count($data['id_item']); $i++)
		{
			...
			DB::table('tbl_inventario_producto')->where('id_inventario',$data['id_item'][$i])
									->where('id_tienda_inventario',$data['id_tienda_inventario'][$i])
									->update([
										'id_estado_producto' => (int)env('PROCESADO_PREREFACCION'),
									]);
		}
	...
	{
		return ModelPrerefaccion::select('id','nombre AS name')->where('estado','1')->get();
	}
	...
	public static function getItemOrden($id_tienda,$id)
	{
        return DB::table('tbl_orden_hoja_trabajo_cabecera')
                                        ...
                                        ->select(
                                                ...
                                                DB::Raw("FORMAT((tbl_orden_item.peso_taller),2,'de_DE') AS peso_taller_total"),
                                                ...
                                                //DB::raw("(SELECT DISTINCT GROUP_CONCAT( DISTINCT cod_bolsas_seguridad SEPARATOR ', ') FROM tbl_contr_cabecera INNER JOIN tbl_orden_hoja_trabajo_detalle AS trabajo_detalle ON tbl_contr_cabecera.codigo_contrato = trabajo_detalle.id_contrato AND tbl_contr_cabecera.id_tienda_contrato = trabajo_detalle.id_tienda_contrato WHERE trabajo_detalle.id_tienda_hoja_trabajo = trabajo_detalle.id_tienda_hoja_trabajo AND trabajo_detalle.id_hoja_trabajo = trabajo_detalle.id_hoja_trabajo) AS Bolsas"),                                             
...
//												'tbl_orden_guardar_items.peso_taller',
  ...
  public static function validarquitarItem($id_tienda_inventario,$id_inventario,$id_contrato)
	...
	/*
	ANULAR ORDEN
	QUITA ITEMS Y REENVIA LA ORDEN A PERFECCIONAMIENTO DE CONTRATOS VENCIDOS
	*/
	public static function AnularOrden($id_inventario,$id_tienda_inventario,$id_contrato,$id_orden)
	{
		...
		try{
			...
			self::deleteItemContratoInv($id_inventario,$id_tienda_inventario);
			self::deleteOrdenItem($id_inventario,$id_tienda_inventario);
			self::updateEstadoOrden($id_tienda_inventario,$id_orden);
			DB::commit();
		}catch(\Exception $e){
			//dd($e);
			if($e->getCode() == 23000)
	...
	public static function updateEstadoInventario($id_inventario,$id_tienda,$motivo,$estado, $peso_taller = 0)
	{
	...
	public static function getOrdenExcel($id_orden, $id_tienda)
    {
        return DB::table('tbl_orden')->join('tbl_orden_hoja_trabajo_cabecera', function($join){
                ...
                ->select(
                        ...
                        'tbl_prod_catalogo.nombre AS nombre',
                        ...
                        'tbl_contr_item_detalle_join.precio_ingresado',
                        //'tbl_inventario_producto.peso_estimado',
                        DB::Raw('(SELECT SUM(tbl_contr_item_detalle.peso_estimado) FROM tbl_contr_item_detalle WHERE tbl_contr_item_detalle.id_tienda = tbl_contr_cabecera.id_tienda_contrato AND tbl_contr_item_detalle.id_codigo_contrato = tbl_contr_cabecera.codigo_contrato) AS peso_estimado'),
                        //DB::RAW('(SELECT SUM(orden_item.peso_total) FROM tbl_orden_item AS orden_item WHERE tbl_orden_item.id_orden = orden_item.id_orden AND tbl_orden_item.id_tienda_orden = orden_item.id_tienda_orden) AS peso_total'),
