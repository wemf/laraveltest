...
namespace App\AccessObject\Nutibara\Refaccion;

use App\Models\Nutibara\OrdenResolucion\OrdenResolucion AS ModelRefaccion;
...
class Refaccion 
{
	public static function get($start, $end, $colum, $order, $search)
	{
		return DB::table('tbl_orden')->leftJoin('tbl_orden_hoja_trabajo_cabecera', function($join){
									...
									->where('tbl_orden.proceso',env('PROCESO_REFACCION'))
	...
	public static function countRefaccion($start, $end, $colum, $order, $search){
	...
	public static function getRefaccionById($id){
		return ModelRefaccion::where('id',$id)->first();
	}
	...
	{
		return ModelRefaccion::select('id','nombre AS name')->where('estado','1')->get();
	}
	...
	public static function getDestinatariosOrdenPadre($id_tienda,$id)
	{
		return DB::table('tbl_orden_destinatario')
				...
				->whereIn('tbl_orden_destinatario.id_orden',$id)
