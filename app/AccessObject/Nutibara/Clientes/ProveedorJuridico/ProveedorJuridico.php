...
namespace app\AccessObject\Nutibara\Clientes\ProveedorJuridico;
...
class ProveedorJuridico 
{
	public static function ProveedorJuridicoWhere($colum, $order,$search){
		return ModelCliente::join('tbl_clie_tipo', function ($join) {
							...
							->select(
								...
								'tbl_cliente.digito_verificacion AS digito_verificacion',
							...
							->where(function ($query) use ($search){
							   ...
							   $query->where('tbl_clie_tipo.id', '=', '6');
							   if($search['tienda'] != "")										
	...
	public static function ProveedorJuridico($start,$end,$colum,$order){
	...
	public static function Create($dataSave,$dataSucursal){
		$result=true;
		try{
			DB::beginTransaction();
			DB::table('tbl_cliente')->insert($dataSave['clienteExistente']);
			DB::table('tbl_clie_suc_cliente')->insert($dataSucursal['crearSucursal']);
			DB::commit();
	...
	public static function actualizarClientes($idTienda,$codigoCliente,$arrayCliente)
	{
			...
			ModelCliente::where('id_tienda',$idTienda)->where('codigo_cliente',$codigoCliente)->update($arrayCliente);	
	}
