...
class CerrarContrato extends NotificacionBase
{   
    ...
    public function SolicitarCerrar()
    {
        $msm=array(
            'val'=>false,
            'msm'=>Messages::$Notificacion['error_anular_contrato']
        );
        $mensaje=Messages::$Notificacion['mensaje_solicitud_cerrado'];
        $accion='contrato/cerrarcontrato/'.$this->codigoContrato.'/'.$this->idTienda.'/'.Auth::id();
    ...
    public function SolicitarReversar()
    {
        ...
        $mensaje=Messages::$Notificacion['mensaje_solicitud_reversa'];
        $accion='contrato/cerrarcontrato/'.$this->codigoContrato.'/'.$this->idTienda.'/'.Auth::id();
        $result=$this->NotificarJefes($mensaje,$accion);
        if($result){
            $msm['val']=true;
    ...
    public function AprobarSolicitud()
    {
        ...
        $mensaje=Messages::$Notificacion['ok_cerrar_contrato_cerrado'];
        $accion='contrato/cerrarcontrato/'.$this->codigoContrato.'/'.$this->idTienda;
        $result=$this->NotificarAuxiliar($mensaje,$accion);
        if($result){
            $msm['val']=true;
            $msm['msm']=Messages::$Notificacion['ok_cerrar_contrato_cerrado'].$this->errorSendeEmail;;
        }
        return $msm;
    }

    public function AprobarSolicitudreverso()
    {
        $msm=array(
            'val'=>false,
            'msm'=>Messages::$Notificacion['error_anular_contrato']
        );
        $mensaje=Messages::$Notificacion['ok_cerrar_contrato_reversado'];
        $accion='contrato/cerrarcontrato/'.$this->codigoContrato.'/'.$this->idTienda;
        $result=$this->NotificarAuxiliar($mensaje,$accion);
        if($result){
            $msm['val']=true;
