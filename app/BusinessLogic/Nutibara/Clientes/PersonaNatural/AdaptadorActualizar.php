...
class AdaptadorActualizar {
    ...
    public static function getPersonaNaturalById($id,$idTienda)
    {
        ...
        $array['datosGenerales'] = PersonaNatural::getPersonaNaturalById($id,$idTienda);
        // dd($array['datosGenerales']);
        $array['estudios'] = PersonaNatural::getEstudiosById($id,$idTienda);
        $array['hist_laboral'] = PersonaNatural::getHistorialLaboralById($id,$idTienda);
        $cantidadParientes = PersonaNatural::getParientesById($id,$idTienda);
        // dd($cantidadParientes);
        $i = 0;
        $j = 0;
        $k = 0;
        // dd($cantidadParientes);
        foreach ($cantidadParientes as $key => $value) { 
            $persona = PersonaNatural::getClienteById($value->codigo_cliente_pariente,$value->id_tienda_pariente);
            // var_dump($persona);
    ...
    public function ordenarDatos($request,$codigoCliente,$id_tienda)
    {
		...
		if(!PersonaNatural::Update($codigoCliente,$id_tienda,$dataSave))
        {
    ...
    public function returnArray($request,$codigo_cliente,$id_tienda)
    {
        ...
        *-- ------------------------------------------------------ -*/
        if(!is_null($request->foto_1) && !is_null($request->foto_2 != NULL))
        {
