...
namespace App\Http\Controllers\Nutibara\MaquilaImportada;
 
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\BusinessLogic\Nutibara\MaquilaImportada\CrudMaquilaImportada;
...
class MaquilaImportadaController extends Controller
{
    public function Index(){
		...
		$urls=array(
			...
			[
				'href'=>'contrato/maquilaimportada',
				'text'=>'Gestión de Contratos'
			],
			[
				'href'=>'contrato/maquilaimportada',
				'text'=>'Maquila Importada'
			]
		);
		return view('MaquilaImportada.index',['urls'=>$urls ,'tienda' => $tienda]);
    }
    ...
    public function get(Request $request){
		...
		$total=CrudMaquilaImportada::getCountMaquilaImportada($search);
		$data=[           
			"draw"=> $draw,
			"recordsTotal"=> $total,
			"recordsFiltered"=> $total,
			"data"=>dateFormate::ToArrayInverse(CrudMaquilaImportada::MaquilaImportada($start,$end,$colum, $order,$search)->toArray())
		];   
	...
	public function Create($id_tienda,$ids_orden){
		...
		$procesos = CrudMaquilaImportada::getListProceso();
		$resultado = CrudMaquilaImportada::DetalleOrdenesMaquilaImportada($id_tienda,$id_orden);
		$urls=array(
			[
				'href'=>'home',
				'text'=>'home'
			...
			[
				'href' => 'contrato/maquilaimportada/procesar/'.$id_tienda.'/'.$ids_orden,
				'text' => 'Procesar Orden de Maquila Importada'
			],
		);
		return view('MaquilaImportada.create',[
										'urls'=>$urls,
										'procesos'=>$procesos,
										'id' =>$ids_orden,
										'items'=>$resultado,
	...
	public function Procesar(Request $request)
	{
		...
		$msm=CrudMaquilaImportada::Procesar($dataSaved);
		if($msm['val']=='Insertado'){
			Session::flash('message', $msm['msm']);
			return $this->generatePDF($msm['ordenes'], $request->id_tienda_orden);
			return redirect('/contrato/maquilaimportada');
		}
	...
	{
		$idx = explode("-",$id);
		$response = CrudMaquilaImportada::getItemOrden($id_tienda,$idx);
	...
	public function GetProveedorById(Request $request)
	{
		$ipValidation = new userIpValidated();
		$tienda = CrudRefaccion::getTiendaByIp($ipValidation->getRealIP());
		$response = CrudMaquilaImportada::GetProveedorById($tienda,$request->nit);
		return  response()->json($response);
	}
