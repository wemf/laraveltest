...
var URL = (function() {
    ...
    return {
        ...
        setEmpleadoActu: function(empleadoA) {
            url.empleadoA = empleadoA;
        },
...
$('form').on('submit', function(e) {
    // Función para validar el tipo de archivos
    if(!submit){
        var valido = false;
        $('input[type="file"]').each(function(){ // seleccionamos los input que cargaran los archivos
            var file = $(this).val(); // validamos lo que contienen los input
            var hidden = $(this).val(); // validamos los archivos existentes al actualizar las imagenes
            var ext = file.substring(file.lastIndexOf(".")); // extraemos la cadena del archivo dejando solo la parte final que nos dará la extención
            
            if(ext != ".jpg" && ext != ".png" && ext != ".jpeg" && ext != "") // verificamos que extención obtuvimos de la cadena
            {
                valido = false;
                Alerta('Error', 'El tipo de archivo que se intenta subir no es una imagen.', 'error');
                return false;
            }else
            {
                valido = true;
        ...
        $.ajax({
           ...
           success: function(datos) {
               if (datos.codigo_cliente !== undefined) {
                   ...
                   if(datos.id_tipo_cliente != 2 && datos.id_tipo_cliente != 1 )
                   {
                    ...
                    $("#confirmSuccess").click(function() {
                        pageAction.redirect(urlBase.make(URL.getEmpleadoActu())+ "/" + datos.codigo_cliente + "/" + datos.id_tienda);
                    });
                   }
                   else
                   {
                        confirm.setSegment('El documento ya existe y pertenece a un '+datos.tipo_cliente+', no es posible cargar información de empelados en los modulos de clientes.');
                        confirm.show();
                        $("#confirmSuccess").click(function() 
                        {
                            $(".ver_doc").val("");
