...
$('#id_pais_exp').change(function() {
    if($('#id_pais_exp').find('option:selected').text() == 'Colombia' && ($('#id_tipo_documento').find('option:selected').text() == 'Visa' || $('#id_tipo_documento').find('option:selected').text() == 'Pasaporte')){
...
$(document).ready(function() {
    ...
    $('.tb1').click(function() {
     ...
     $("#numero_documento").blur(function() {
         ...
         if (identificacion != "") {
             $.ajax({
                url: URL.getEmpleado().trim() + "/" + identificacion,
                type: "get",
                success: function(datos) {
                    if (datos.codigo_cliente !== undefined) {
                        confirm.setTitle('Aviso');
                        confirm.setSegment('El empleado ya existe en el sistema, ¿desea cargar su información?');
                        confirm.show();
                        confirm.setFunction(function() {
                            URL.setEmpleadoActu(URL.getEmpleadoActu().trim() + "/" + datos.codigo_cliente + "/" + datos.id_tienda);
                        });
                        $("#confirmSuccess").click(function() {
                            pageAction.redirect(URL.getEmpleadoActu().trim());
                        });
                    }
                }
            })
