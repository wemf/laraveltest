...
$('#btn-solicitar-procesarJZ').click(function () {
    var cont_reclasificado = 0;
    $('.val-reclasificado').each(function () {
        if ($(this).val() == 0) {
            ++cont_reclasificado;
    ...
    if (valClass('peso_libre') && cont_reclasificado == 0) {
        $.ajax({
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
            url: urlBase.make('contrato/vitrina/solicitud/procesarJZ'),
            type: "GET",
            data: {
                id_tienda: $('#id_tienda_ordenes').val(),
                id_ordenes: $('#numero_orden').val(),
...
$('#btn-guardar').click(function () {
    ...
    $.ajax({
        ...
        url: urlBase.make('contrato/vitrina/guardar'),
        type: "GET",
        data: {
            id_tienda: $('#id_tienda_ordenes').val(),
            id_ordenes: $('#numero_orden').val(),
        ...
        success: function (datos) {
            ...
            if (datos.val) {
                setTimeout(function () {
                    window.location = urlBase.make('contrato/vitrina')
                }, 3000);
                Alerta('Información', datos.msm, 'success');
            }else{
                setTimeout(function () {
                    window.location = urlBase.make('contrato/vitrina')
                }, 3000);
...
$('#btn-rechazar').click(function () {
    ...
    $.ajax({
        ...
        url: urlBase.make('contrato/vitrina/rechazar'),
        type: "GET",
        data: {
            id_tienda: $('#id_tienda_ordenes').val(),
            id_ordenes: $('#numero_orden').val(),
                    ...
                    jQuery.each(datos.data, function (indice, valor) {
                        ...
                        html_tabla +=
                            ...
                            html_tabla +=`
                                ...
                                <td>${ ((datos.data[indice].peso_final != null) ? datos.data[indice].peso_final : "") }</td>
...
$(document).ready(function(){
    ...
    $('#btn_reporte_pdf').click(function () {
        ...
        if (id_orden != "" && id_orden != undefined) {
            ...
            $("#frm_reporte_pdf").attr("action", urlBase.make('contrato/refaccion/generatepdf'));
    ...
    // fin funciones
    $('#btn-aceptarJZ').click(function(){
        $('#btn-solicitar-procesarJZ').removeClass('hide');
        $('#btn-aceptarJZ').hide();
        $('#btn-rechazarJZ').hide();
        $('#reclass').attr('disabled',false);
    })
...
function btnProcesar(ver = null) {
    ...
    $('.table_resolucion .check-resolucionar:checked').each(function () {
        ...
        ++cont;
        estado = $(this).parent().parent().parent().find('td.var_estado').text().trim();
        if (estado == "Cerradas" || estado == "Anuladas" || estado == "Procesado") paso = false;
        if (estado == "Pendiente Por Procesar" && $('#rol_user').val() != $('#role_jefe_zona').val() && $('#rol_user').val() != $('#rol_administrador_joyeria').val()) paso = false;
        if (estado == "Pendiente por aprobación admin de la Joyeria" && $('#rol_user').val() != $('#rol_administrador_joyeria').val()) paso = false;
        if (estado == "Abierta pendiente de aprobación de jefe de Zona" && $('#rol_user').val() != $('#role_jefe_zona').val()) paso = false;
    });
    ...
    if (paso) {
        ...
        if (estado == "Pendiente Por Procesar" && $('#rol_user').val() != $('#role_jefe_zona').val() && $('#rol_user').val() != $('#rol_administrador_joyeria').val()){
            Alerta('Alerta', 'Solo los contratos abiertos se pueden procesar.', 'warning');
        }
        else if (estado == "Pendiente por aprobación admin de la Joyeria" && $('#rol_user').val() != $('#rol_administrador_joyeria').val()){
            Alerta('Alerta', 'Solo el administrador de joyeria puede procesar esta orden.', 'warning');
        }
        else if (estado == "Abierta pendiente de aprobación de jefe de Zona" && $('#rol_user').val() != $('#role_jefe_zona').val()) {
            Alerta('Alerta', 'Solo el jefe de zona puede procesar esta orden.', 'warning');
        }
    }
}
    ...
    $.ajax(
    {
        ...
        success: function (datos) {
            ...
            jQuery.each(datos, function (indice, datos) {
                //autorizado por eduardo seide 2:22:06 pm 25/09/2018
                if ($('#anterior').val() == "9" || $('#anterior').val() == "12")
                {
                // if(datos.set_valor == '1'){
                        ...
                        (
                            '<div class="form-group bottom-20 nth-child-attribute-' + datos.id_atributo_padre + '" id="form-group-' + datos.id_atributo + '">'+
                                ...
                                '<div class="col-md-6 col-sm-6 col-xs-12">'+
                                    '<select id="slc-attr-' + datos.id_atributo + '" data-concat="' + datos.concatenar_nombre + '" onchange="loadAttributeAttributes(' + datos.id_atributo + ', this.value, \'' + URL.getUrlAttributeAttributesById() + '\')" class="form-control col-md-7 col-xs-12 requiered">'+
                    ...
                    if (datos.nombre_atributo == "Calidad")
                    {
                        selected = (datos.valor_seleccionado == 1) ? ' selected ' : '';
                        if ($select == datos.id_atributo && $option != datos.id_valor) {
                            $('#slc-attr-' + datos.id_atributo).append(
...
function reclasificarItemJZ(id_tienda, codigo_contrato, id_inventario, id_linea_item) {
    $('.selects').html('');
    $('#parentAttribute').find('option').remove();
    $('#var_id_tienda').val(id_tienda);
    $('#var_id_inventario').val(id_inventario);
...
function totales_resolucion() {
    var total_valor_contratos = 0,
        ...
        cantidad_ids = 0;
        total_cantidad_ids = 0;

    $('.totales_resolucion tbody tr').each(function () {
        if (!$(this).find('td').hasClass('dataTables_empty')) {
            ...
            peso_taller = ($(this).find('td.var_peso_taller').text() != '') ? parseFloat($(this).find('td.var_peso_taller').text()) : 0;
            peso_final = ($(this).find('td.var_peso_final').text() != '') ? parseFloat($(this).find('td.var_peso_final').text()) : 0;
            ...
            cantidad_ids = ($(this).find('td.var_ids_internos').text() != '') ? parseInt($(this).find('td.var_ids_internos').text()) : 0;
            ...
            cantidad_ids = (cantidad_ids == NaN) ? 0 : cantidad_ids;
            if ($(this).val() == '1') {
                ...
                total_cantidad_ids -= cantidad_ids;
                --total_contratos;
        ...
        $('#lbl_total_ids').text(((total_cantidad_ids).toString()));
...
function datatableVitrina(id_table, url_ajax, url_lenguage, columns) {
    table_multi_select = $('#' + id_table).DataTable({
        ...
        "fnDrawCallback": function (oSettings) {
            $('#' + id_table + ' tbody td:not(.no-replace-spaces)').text(function () {
...
$('#btn_anular').click(function () {
    ...
    //obtener orden
    id_orden = $('#table_refaccion .check-resolucionar:checked').parent().parent().parent().attr('id');
    ...
    if (id_orden != "" && id_orden != null && id_orden != undefined) {
        $.ajax({
            ...
            url: urlBase.make('contrato/refaccion/anular'),
            type: "POST",
            data: {
                id_tienda: tienda,
                id_orden: id_orden
