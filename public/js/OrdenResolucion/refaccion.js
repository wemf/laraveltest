...
$('#btn_reporte_excel').click(function(){
    ...
    if(id_orden != "" && id_orden != undefined){
        ...
        pageAction.redirect(urlBase.make(`contratos/resolucionar/excelrefaccion/${ id_orden }/${ id_tienda }/refaccion`));
    }else{
        Alerta('Error', 'Seleccione un registro.', 'error')
    }
});
...
var refaccion = (function(){
    return {
        detalles_tabla: function () {
            $('#table_refaccion tbody').on('click', 'td.details-control', function () {
        ...
        detalle_tabla_html: function (codigo_orden, id_tienda) {
            ...
            $.ajax({
                ...
                success: function (datos) {
                    ...
                    html_tabla += '<th>Atributos</th><th>Descripción</th><th>Peso total</th><th>Peso estimado</th><th>Peso taller</th><th>Nro. de Contrato</th></thead>'
...
function totales_resolucion_det(){
    var 
        ...
        $('#table_refaccion tbody tr').each(function(){
            ...
            if($('#table_refaccion tbody tr').find('.dataTables_empty').length == 0)
                ++total_contratos;
        });
...
$(document).ready(function(){
    ...
    // $('#table_refaccion tbody').on('click', 'tr', function () {
