...
$('#btn-quitar').click(function(){
    ...
    if (itemSelec != "")
    {
        $.ajax({
            ...
            url: urlBase.make('contrato/maquila/validarItem'),
            type: "POST",
            data: {
...
$('#confirmar').click(function(){
    ...
    if (itemSelec != "abc")
    {
        $.ajax({
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
            url: urlBase.make('contrato/maquila/quitarItems'),
            type: "POST",
            data: {
                items: itemSelec
            },
            success: function(datos){
                ...
                if (datos == "exito") {
                    ...
                    pageAction.redirect(urlBase.make('contrato/maquila'));
                } else if(datos="error") {
...
$('#btn_reporte_excel').click(function(){
    ...
    id_orden = $('#table_maquila .check-resolucionar:checked').parent().parent().parent().attr('id');
    if(id_orden != "" && id_orden != undefined){
        Alerta('Cargando', 'Descargando Excel..', 'notice');
        pageAction.redirect(urlBase.make(`contratos/resolucionar/excelmaquila/${ id_orden }/${ id_tienda }/maquila`));
    }else{
        Alerta('Error', 'Seleccione un registro.', 'error')
    }
});
...
$('#btn_reporte_pdf').click(function(){
    ...
    if(id_orden != "" && id_orden != undefined){
        ...
        $("#frm_reporte_pdf").attr("action", urlBase.make('contrato/maquila/generatepdf'));
...
var maquila = (function(){
    return {
        detalles_tabla: function () {
            $('#table_maquila tbody').on('click', 'td.details-control', function () {
                ...
                if (row.child.isShown()) {
                    ...
                    // Open this row
                    row.child(maquila.detalle_tabla_html(codigo_orden, id_tienda)).show();
                    tr.addClass('shown');
                }
            });
        },
        detalle_tabla_html: function (codigo_orden, id_tienda) {
            ...
            var cant_cols=0;
            $.ajax({
                url: urlBase.make('contrato/maquila/getItemOrden/' + id_tienda + '/' + codigo_orden),
                type: "get",
                async: false,
                success: function (datos) {
                    console.log(datos);
                    ...
                    html_tabla += '<th>Atributos</th><th>Descripción</th><th>Peso total</th><th>Peso estimado</th><th>Peso taller</th></thead>'
...
function totales_resolucion_det(){
    var 
        ...
        $('#table_maquila tbody tr').each(function(){
...
$(document).ready(function(){
    ...
    // $('#table_maquila tbody').on('click', 'tr', function () {
...
function saveOrden(url){    
    ...
    $("#form_maquila").attr("action", urlBase.make(url));
    $('#form_maquila').submit();
