...
var vcliente = (function () {
    return {
        clienteManual: function () {
            ...
            if (completo) {
                var parametros = $('#id_tienda').val() + "/" + $('#saldo_favor').val() + "/" + $('#codigo_plan').val() + "/" +$('#tipodocumento_manual').val() + "/" + $('#numdocumento').val();
                pageAction.redirect(urlBase.make(`generarplan/${ parametros }`));
            }
        }
    }
})();
